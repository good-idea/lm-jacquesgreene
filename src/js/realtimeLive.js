...
$('.scrollTo').map((i, el) => {
	const scroll = new ScrollTo(el, publisher);
	// scroll.autoCallback = components.player.play.bind(player);
	scroll.setAuto();
	if (!components.scrolls) components.scrolls = [];
	components.scrolls.push(scroll);
});

$('.build-media').map((i, el) => {
	const media = mediaBuilder(el, publisher);
	if (!components.media) components.media = [];
	components.media.push(media);
});
